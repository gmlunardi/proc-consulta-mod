...
// if AJAX request (triggered by deletion via admin grid view), we should not redirect the browser
    ...
    public function printaCursistas($id, $ed){
        $variosCurs = Cursista::model()->findAllByAttributes(array('municipio_id' => $id, 'edicao' => $ed));
        $resultShow = '';
        if (!empty($variosCurs)){
            
            foreach ($variosCurs AS $curs){
                $resultShow .= $curs->nome.'<br/>';
            }
        } else {
            $resultShow = "Não existem cursistas concluintes.";
        }
        return $resultShow;
    }
